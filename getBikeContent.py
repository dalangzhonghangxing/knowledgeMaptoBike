...
def divideKnowledgre(knowledge):
    # 首先按照并列词划分
    partition = []
    ...
    for p in partition:
        ...
        if subject != "":
            if bike.isBikeWords(subject):
                ...
                combination.append(subject + left)
                ...
                if len(candinates) > 0:
                    ...
                    for candinate in candinates:
                        combination.append(candinate + left)
...
def getUnmatchedContent():
    ...
    Utils.writeFile("mapping.txt", mapping)
...
# getUnmatchedContent()
print(getUnmatchedKnowledgeContent("含绝对值符号的一元一次方程"))
